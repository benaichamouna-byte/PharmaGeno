...
# SEULE VRAIE MODIFICATION vs le script original : le fichier source.
# Tout le reste (architecture, hyperparametres, procedure) est identique.
DATASET_PATH = DATA_DIR + '/phenotype_drug_dataset_complet.csv'   # <- doit deja etre le fichier corrige
                                                                    #    (celui remplace plus tot cette semaine)
...
print('')
print('='*60)
print('K-FOLD 5 FOLDS — Dataset CORRIGE, 31 genes, Morgan FP')
print('='*60)
...
print('')
print('='*60)
print('RESULTATS FINAUX (DATASET CORRIGE)')
print('='*60)
...
print('')
print('A COMPARER AUX ANCIENS RESULTATS (dataset original, non corrige) :')
print('DL=0.834+/-0.018  XGB=0.881+/-0.003  RF=0.795+/-0.004')

# Entrainement final — SORTIE SEPAREE, ne touche pas aux modeles de production actuels
...
joblib.dump(rf_f,  RESULTS_DIR+'/rf_pgx_31genes_CORRIGE.pkl')
joblib.dump(xgb_f, RESULTS_DIR+'/xgb_pgx_31genes_CORRIGE.pkl')
torch.save(dl_f.state_dict(), RESULTS_DIR+'/dl_pgx_31genes_CORRIGE.pth')
with open(RESULTS_DIR+'/le_pgx_31genes_CORRIGE.pkl','wb') as f: pickle.dump(le,f)
with open(RESULTS_DIR+'/genes_pgx_31genes_CORRIGE.pkl','wb') as f: pickle.dump(GENES,f)
with open(RESULTS_DIR+'/scaler_pgx_31genes_CORRIGE.pkl','wb') as f: pickle.dump(scaler,f)
print('')
print('Modeles CORRIGES sauvegardes sous *_CORRIGE — les fichiers de production (*_31genes.*) ne sont PAS touches.')
print('Une fois les resultats confirmes bons, on remplacera les fichiers de production manuellement.')
